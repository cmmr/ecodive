...
The metrics implemented natively in `ecodive` are highly optimized for both
speed and memory. However, to provide the broadest range of metrics through the
same interface, `ecodive` also provides wrappers around functions from the
`vegan` and `tabula` R packages. To use them, `vegan` and/or `tabula` must be
...
* Classic metrics (via `vegan`): `fisher()`, `ace()`

* Classic metrics (via `tabula`): `brillouin()`, `mcintosh()`, `berger()`, `margalef()`, `menhinick()`, `squares()`
...
* Classic metrics (via `vegan`): `clark()`, `alt_gower()`, `morisita()`, `horn()`, `mountford()`, `raup()`, `binomial()`, `cao()`, `mahalanobis()`, `chisq()`, `chord()`, `hellinger()`, `aitchison()`, `robust_aitchison()`
